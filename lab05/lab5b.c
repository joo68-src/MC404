...
// ENCODERS
unsigned int en_R(const InstData *d) {
    return ((unsigned int)d->funct7 << 25) | 
            ((unsigned int)d->rs2 << 20) |
            ((unsigned int)d->rs1 << 15) |
            ((unsigned int)d->funct3 << 12) |
            ((unsigned int)d->rd) << 7 |
            (unsigned int)d->opcode;
}

unsigned int en_I(const InstData *d) {
    unsigned int i = (unsigned int)(d->imm & 0xFFF);
    return (i << 20) | 
    ((unsigned int)d->rs1 << 15) |
    ((unsigned int)d->funct3 << 12) |
    ((unsigned int)d->rd << 7) | 
...
unsigned int en_S(const InstData *d) {
    unsigned int i = (unsigned int)(d->imm & 0xFFF);
    return ((i >> 5) << 25) | 
    ((unsigned int)d->rs2 << 20) |
    ((unsigned int)d->rs1 << 15) |
...
unsigned int en_B (const InstData *d) {
    unsigned int u = (unsigned int)(d->imm & 0x1fff);
    unsigned int enc = 0;
    enc |= ((u >> 12) & 1) << 31; //imm[12]
    enc |= ((u >> 5) & 0x3f) << 25; //imm[10:5]
    enc |= ((unsigned int)d->rs2 << 20);
    enc |= ((unsigned int)d->rs1 << 15);
    enc |= ((unsigned int)d->funct3 << 12);
    enc |= ((u >> 1) & 0xf) << 8; // imm[4:1]
    enc |= ((u >> 11) & 1) << 7; // imm[11]
    enc |= (unsigned int)d->opcode;
    return enc;
}

unsigned int en_U (const InstData *d) {
    unsigned int i = (unsigned int)(d->imm & 0xfffff);
    return (i << 12) |
    ((unsigned int)d->rd << 7) |
    (unsigned int)d->opcode;
}

unsigned int en_J (const InstData *d) {
    unsigned int u = (unsigned int)(d->imm & 0x1fffff); 
    unsigned int enc = 0;
    enc |= ((u >> 20) & 1) << 31;
    enc |= ((u >> 1) & 0x3ff) << 21;
    enc |= ((u >> 11) & 1) << 20;
    enc |= ((u >> 12) & 0xff) << 12;
    enc |= ((unsigned int)d->rd << 7);
    enc |= (unsigned int)d->opcode;
    return enc;
}

unsigned int pack (const InstData *d) {
    switch (d->type) {
        case R: return en_R(d);
        case I: return en_I(d);
        case S: return en_S(d);
        case B: return en_B(d);
        case U: return en_U(d);
        case J: return en_J(d);
        default: return 0;
    }
}
...
int main()
{
    ...
    InstData data;
    get_inst_data(str, &data);
    unsigned int encoded = pack(&data);
    hex_code((int)encoded);
    return 0;
}
